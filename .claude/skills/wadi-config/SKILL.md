...
- **Drawings → house (recreate an existing design).** **Do NOT try to read
  dimensions off an architect's drawing yourself — it's unreliable and produces the
  wrong house.** Instead run a **guided dialog**: the USER reads their drawing and
  describes each piece in plain terms (names, sizes in feet, where things sit), and
  YOU do all the coordinate / scale / convention work, building **one room at a
  time** and rendering after each so they confirm. Ask **spatially** ("how big?
  which corner? next to what?"), never for coordinates. Never build the whole plan
  from one answer. Full method in `prompts/sketch-to-config.md`.
