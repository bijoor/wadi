...
> New here? Read §1 to §5 in order and you can model a real house. The later sections
> (parametrics, components, libraries, the configurator) are how you make that
> house reusable and adjustable.
>
> Companion chapters: [the concept](01-concept.md) (how Wadi uses a DSL),
> [components & libraries](04-components-and-libraries.md) (reuse in detail),
> and [the parametric method](06-the-method.md) (the method behind it).
> New to the whole thing? Start at the [documentation index](README.md).
...
Paste this into the editor. It is the smallest thing that renders:
...
You get a single room, enclosed on all four sides, on one floor. That is a valid
...
- X → right, Y → down. Think of the floor plan as you would draw it on paper:
...
- Units are abstract. Geometry is stored in plain "project units". The `units`
  ...
  `per_unit 10`, a wall drawn `108` units tall is annotated 10'-9". If you
...
- `center` (use this): the numbers are wall centrelines. Two rooms that
  share a wall use the same coordinate on their shared edge, so they abut
...
- `outer` (legacy): the numbers are the outer wall face. Adjacent rooms
  must overlap by a wall thickness, so you are constantly adding and subtracting
...
roofs, component instances. The rest of §5 to §11 is that vocabulary.

---

## 5. Rooms, walls & openings

The `room` is the main object. It is a rectangle that grows its own walls.
...
- Omit a side to leave it open. That is how you model a verandah or a
  sit-out.
...
The model: a roof is made of segments, each an axis line
...
Choosing a roof by type: `flat` (add a `parapet`); `shed`
...
Everything so far is a fixed house. The next step is making it a
...
The formula language is small: arithmetic only:
...
- `with { p = v, … }` overrides params; un-overridden ones fall back to defaults.
  ...
  house's variables.
- A component expands byte-identical to writing its objects inline, so there is no
  runtime cost.

Components nest: a component may `use` another component and place
...
A library is a `.wdl` file whose top level holds `component` / `asset`
...
into a bounded template. Each knob targets a `var` by name; turning it
writes a number into that variable and the whole house re-flows.
...
it in the app and turns your knobs to fit their plot and taste, without touching the
geometry.
...
**Layers** are display groups (show/hide, colour), visual only, never geometry:
...
without first-class syntax yet. Every model type now has its own syntax, so you
...
[**components & libraries**](04-components-and-libraries.md) chapter, the example files in
[`wadi-dsl/examples/`](../wadi-dsl/examples/) (start with `minimal.wdl`, then `coastal.wdl`
for a grid-driven house and `complete.wdl` for every construct at once), and the design
philosophy in [**the parametric method**](06-the-method.md).
```
