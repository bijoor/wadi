#!/usr/bin/env python3
"""
Test render with adjusted texture scale
"""
...
print("\n" + "="*70)
print("TEST RENDER WITH UV MAPPING AND ADJUSTED SCALE")
print("="*70)
print(f"Laterite walls: 15.0x scale")
print(f"Terracotta roof: 50.0x scale")
print(f"UV unwrapping: ENABLED (fixes texture application)")
print("="*70)
print()
...
def create_laterite_material():
    ...
    if os.path.exists(LATERITE_TEXTURE_PATH):
        print(f"✓ Using laterite: {os.path.basename(LATERITE_TEXTURE_PATH)} at scale 15x")
        ...
        node_mapping.inputs['Scale'].default_value = (15.0, 15.0, 15.0)  # Object coords scale

        # Use Object coordinates (better for architectural geometry)
...
def create_terracotta_material():
    ...
    if os.path.exists(TERRACOTTA_TEXTURE_PATH):
        print(f"✓ Using terracotta: {os.path.basename(TERRACOTTA_TEXTURE_PATH)} at scale 50x")
        ...
        node_mapping = nodes.new(type='ShaderNodeMapping')
        node_mapping.location = (-600, 0)
        node_mapping.inputs['Scale'].default_value = (50.0, 50.0, 50.0)  # Object coords scale

        # Use Object coordinates for roof
        links.new(node_texcoord.outputs['Object'], node_mapping.inputs['Vector'])
        links.new(node_mapping.outputs['Vector'], node_img.inputs['Vector'])
        links.new(node_img.outputs['Color'], node_bsdf.inputs['Base Color'])
...
print("\nApplying materials and UV unwrapping...")
walls_applied = []
roofs_applied = []
...
for obj in bpy.data.objects:
    ...
    # UV unwrap the object for proper texture mapping
    ...
    # Apply terracotta to roof
        ...
        if terracotta_mat:
            ...
            roofs_applied.append(obj.name)

print(f"\n✓ Applied laterite to {len(walls_applied)} walls:")
for name in walls_applied[:5]:  # Show first 5
    print(f"  - {name}")
if len(walls_applied) > 5:
    print(f"  ... and {len(walls_applied) - 5} more")

print(f"\n✓ Applied terracotta to {len(roofs_applied)} roof objects:")
for name in roofs_applied:
    print(f"  - {name}")
...
if os.path.exists(output_path):
    ...
    print("\nCompare this with the previous render to see texture detail improvement!")
else:
    print("\n✗ Render failed")
