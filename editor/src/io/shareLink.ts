...
export async function encodeConfigToHash(config: HouseConfig): Promise<string> {
  // Preview snapshots are fat data: URLs and NOT model data — strip them so
  // share links stay short (they're regenerated by the architect, not shared).
  const { thumbnail: _t, thumbnails: _ts, ...slim } = config as HouseConfig & {
    thumbnail?: string;
    thumbnails?: string[];
  };
