  ...
  if ((hc as { coord_convention?: string }).coord_convention === "center" && hc.floors) {
    ...
    for (const fl of hc.floors) {
      for (const o of (fl.objects ?? []) as Obj[]) {
        ...
        // Roofs are intentionally NOT grown here. A roof segment's `width` and
        // ridge endpoints already describe the OUTER footprint (configs author
        // `width = House.W`, ridge `0 → House.L`) — the same outer face the floor
        // slab reaches after its own +t growth. Growing the roof again would push
        // it half a wall-thickness beyond every wall face (a 27' house getting a
        // 27'10" roof, inconsistent with the floor plan). The ring beam is inset
        // to the wall centreline in the roof derivation (roof/v2/segments.ts) and
        // the overhang extends out from the outer face from there.
      }
    }
  }
