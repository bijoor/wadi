...
// PWA: register the service worker so the browser app installs to the home
// screen and works offline (mobile / tablet, where there's no Tauri build). The
// worker + manifest live next to index.html at /app/. Guarded to a secure
// browser context — skips the Tauri desktop app (custom protocol, already
// offline) and file:// — and failures are swallowed so registration can never
// break app start-up. `sw.js` is generated by the viewer build (pwaBuild).
function registerServiceWorker(): void {
  if (!("serviceWorker" in navigator)) return;
  const secure = location.protocol === "https:" || location.hostname === "localhost";
  if (!secure) return; // Tauri (tauri://) / file:// → native offline, no SW
