...
export function LayersEditor() {
  ...
  // The full effective set, INCLUDING the code-managed roof layers — they show
  // as a read-only "Roof" group whose ORDER (only) can be changed.
  const layers: LayerDef[] = useMemo(
    () => (config ? effectiveLayers(config) : []),
    [config],
  );
      ...
      >
        ...
        <div className="overflow-y-auto px-5 py-4">
          <p className="mb-3 text-[11px] leading-relaxed text-slate-400">
            ...
            order (top&nbsp;→&nbsp;bottom: roof, floors, site). The <b>Roof</b> group is
            managed automatically — you can reorder it, but its layers aren't editable.
          </p>
          ...
          {groups.map((g, gi) => {
            ...
            // The roof group is code-managed: its components can't be edited,
            // but the group as a whole can be reordered.
            const roofGrp = g.layers.length > 0 && g.layers.every((l) => isRoofLayer(l.id));
            return (
              <div key={g.name} className="mb-3 rounded border border-slate-800">
                <div className="flex items-center gap-1.5 bg-slate-800/60 px-3 py-1.5">
                  ...
                  {roofGrp ? (
                    <span className="min-w-0 flex-1 px-1 py-0.5 text-[12px] font-semibold text-slate-100">
                      {g.name}
                ...
                {!isCol && (
                  <div className="space-y-1.5 px-3 py-2.5">
                    {g.layers.map((l, li) =>
                      roofGrp ? (
                        // Read-only roof layer: swatch + label, no editing.
                        <div key={l.id} className="flex items-center gap-2 opacity-90">
                          ...
                          />
                          <span className="min-w-0 flex-1 rounded bg-slate-800/40 px-1.5 py-1 text-[12px] text-slate-300">
                            {l.label}
                          </span>
                        </div>
                      ) : (
